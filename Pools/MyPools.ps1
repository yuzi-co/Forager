...
if ($Querymode -eq "Core") {
    $Pools = @(
        ...
        [PSCustomObject]@{ Coin = "Pascal"       ; Symbol = "PASC" ; Algo = "RandomHash2" ; Server = "mine.pool.pascalpool.org"     ; Port = 3333 ; Fee = 0.005 ; User = $Wallets.PASC + "$(if ($null -eq ($Wallets.PASC -split '\.')[1]) {'.0'}).#WorkerName#/#Email#" }
