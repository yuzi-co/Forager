...
$MinerReport = ConvertTo-Json @($ActiveMiners.SubMiners | Where-Object Status -eq 'Running' | ForEach-Object {
        ...
        [PSCustomObject]@{
            ...
            Path           = $M.Pool.Symbol + $(if ($M.AlgorithmDual) {'_' + $M.PoolDual.Symbol})
            ...
            Pool           = $M.Pool.PoolName + '-' + $M.Pool.Location + $(if ($M.AlgorithmDual) {"/$($M.PoolDual.PoolName + '-' + $M.PoolDual.Location)"})
